...
class AdminManageCityHelpFragment : Fragment() {
    ...
    private fun setupToolbar() {
        binding.adminManageToolbar.title = "Manage CityHelp"
    ...
    private fun fetchCityHelp() {
        binding.adminManageProgressBar.visibility = View.VISIBLE
        db.collection("cityhelp").get()
            .addOnSuccessListener { documents ->
                binding.adminManageProgressBar.visibility = View.GONE
                val list = documents.map { doc ->
                    AdminManageModel(
                        id = doc.id,
                        title = doc.getString("serviceName") ?: doc.getString("category") ?: "N/A",
                        subtitle = doc.getString("contact") ?: "N/A",
                        type = "cityhelp"
                    )
                }
